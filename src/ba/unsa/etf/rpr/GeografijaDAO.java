...
public class GeografijaDAO {
    ...
    private GeografijaDAO() {
        ...
        try {
            glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.univerzitetski, grad.naziv_univerziteta FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        } catch (SQLException e) {
            regenerisiBazu();
            try {
                glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.univerzitetski, grad.naziv_univerziteta FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        ...
        try {
            ...
            promijeniGradUpit = conn.prepareStatement("UPDATE grad SET naziv=?, broj_stanovnika=?, drzava=?, univerzitetski=?, naziv_univerziteta=? WHERE id=?");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private Grad dajGradIzResultSeta(ResultSet rs) throws SQLException {
        int univerzitetski = rs.getInt(5);
        Grad grad;
        if (univerzitetski == 1)
            grad = new UniverzitetskiGrad(rs.getInt(1), rs.getString(2), rs.getInt(3), null, rs.getString(6));
        else
            grad = new Grad(rs.getInt(1), rs.getString(2), rs.getInt(3), null);
    ...
    public void dodajGrad(Grad grad) {
        try {
            ...
            if (grad instanceof UniverzitetskiGrad) {
                ...
                dodajGradUpit.setString(6, ug.getNazivUniverziteta());
            } else {
    ...
    public void izmijeniGrad(Grad grad) {
        try {
            ...
            if (grad instanceof UniverzitetskiGrad) {
                ...
                promijeniGradUpit.setString(5, ug.getNazivUniverziteta());
            } else {
